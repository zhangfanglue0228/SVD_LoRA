...
@dataclass
class SVDLora_res_v2_Config(PeftConfig):
    """
    lue
    """
    ...
    def __post_init__(self):
        self.peft_type = PeftType.SVDLORA_res_v2


class SVDLora_res_v2_Model(torch.nn.Module):
    """
    lue
    """
...
class Linear(nn.Linear, LoraLayer):
    ...
    def train(self, mode: bool=True):
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            # if self.r > 0:
            self.weight.data += (
                transpose(self.lora_B.weight @ self.lora_sigma.weight @ self.lora_A.weight, fan_in_fan_out=self.fan_in_fan_out) * self.scaling * torch.sigmoid(self.coefficient)
            )
            self.merged = True
            print("Merged!")
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # if self.r > 0:
            result += (((self.lora_dropout(x.to(self.lora_A.weight.dtype)) @ self.lora_A.weight.T) @ self.lora_sigma.weight.T) @ self.lora_B.weight.T) * self.scaling * torch.sigmoid(self.coefficient)
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias) * torch.sigmoid(self.coefficient)
